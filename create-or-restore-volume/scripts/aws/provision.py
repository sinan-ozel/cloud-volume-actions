"""AWS EBS volume provisioning script.

Creates a new EBS volume or restores from the most recent snapshot.
...
def main():
    ...
    volumes = sorted(
        response.get("Volumes", []), key=itemgetter("CreateTime"), reverse=True
    )
    ...
    if volumes:
        ...
        snapshots = sorted(
            response.get("Snapshots", []), key=itemgetter("StartTime"), reverse=True
        )
